...
class IntentRouter:
    ...
    def classify_intent(message_text: str, has_photo: bool = False, has_document: bool = False) -> Dict[str, Any]:
        ...
        if has_photo:
            return {
                "intent": "FOOD_NUTRITION_ANALYSIS" if any(w in text for w in ["еда", "обед", "ужин", "завтрак", "калории", "кушать"]) or not text else "RECEIPT_OCR",
                "primary_agent": "health" if any(w in text for w in ["еда", "обед", "ужин", "завтрак", "калории"]) or not text else "documents",
                "secondary_agents": ["finance"] if not any(w in text for w in ["еда", "обед", "ужин", "завтрак"]) else [],
            }
        ...
        if any(w in text for w in ["сон", "оура", "oura", "пульс", "готовность", "readiness", "здоровье", "самочувствие", "анализ"]):
            return {
        ...
        if any(w in text for w in ["потратили", "расходы", "бюджет", "купили", "чек", "цена", "стоит", "деньги", "грн", "долларов"]):
            return {
        ...
        if any(w in text for w in ["напомни", "задача", "список покупок", "купить", "план", "встреча", "стоматолог", "календарь"]):
            return {
